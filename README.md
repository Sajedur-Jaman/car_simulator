...
git clone https://github.com/Sajedur-Jaman/car_simulator.git
cd car_simulator
...
```bash
jar cfe CarRacingGame.jar CarRacingGame *.class
...
| System | Details |
|---|---|
| **Game Loop** | `javax.swing.Timer` fires every `1000/60 ms` (~60 FPS), calling `update()` then `repaint()` |
| **Collision Detection** | Axis-aligned bounding box (AABB) intersection between the player and all enemies/items |
| **Road Curves** | A sinusoidal `curveOffset` smoothly shifts the road left/right at random intervals |
| **Scoring** | +10 pts per enemy car passed, +5 pts per gas canister collected |
| **Win Condition** | Reach `distanceTraveled >= 5000` units |
| **Environment Parallax** | Background layers scroll at different fractions of player speed for depth |
